...
async function load() {
  try {
    detail.value = await store.detail(id)
    error.value = ''
  } catch (e) {
...
async function addCheck() {
  if (!detail.value) return
  await store.createCheck(id, check as never)
  checkModal.value = false
  Object.assign(check, { name: '', type: 'ping', target: '', port: 0, expected_status: 0, enabled: true })
  await load()
}

function openEdit() {
  if (!detail.value) return
  Object.assign(edit, detail.value.device)
  editModal.value = true
}

async function saveEdit() {
  await store.updateDevice(id, edit)
  editModal.value = false
  await load()
}

async function removeCheck(checkID: number) {
  await store.deleteCheck(id, checkID)
  await load()
}
